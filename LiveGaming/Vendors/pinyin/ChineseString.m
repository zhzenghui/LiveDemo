...
{
    NSMutableArray *tempArray = [self ReturnSortChineseArrar:stringArr friends:friends];
...
//         NSString *string = ((ChineseString*)object).string;
         Friendlist *friend = ((ChineseString*)object).friend;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
            item = [NSMutableArray array];
            [item  addObject:friend];
...
//返回排序好的字符拼音
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr friends:(NSMutableArray *)friends
{
    ...
    for(int i=0;i<[stringArr count];i++)
    {
       ...
       // NSLog(@"string====%@",chineseString.string);
        ...
        if ([predicate evaluateWithObject:chineseString.string])
        {
         ...
         chineseString.friend = (Friendlist*)friends[i];
