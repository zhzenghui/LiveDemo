...
#define kNavigationHeight 0.0 // Default tab height
// 标题所占的高度
#define kDefaultTabHeight 30.0 // Default tab height
#define kDefaultTabOffset 0.0 // Offset of the second and further tabs' from left
// 字体所占的宽度
...
- (void)viewWillLayoutSubviews {
    ...
    frame.origin.x = 0.0;
    frame.origin.y = kNavigationHeight;//self.tabLocation ? 0.0 : self.view.frame.size.height - self.tabHeight;
    ...
    frame.origin.x = 0.0;
    frame.origin.y = kNavigationHeight + self.tabHeight;//self.tabLocation ? self.tabHeight : 0.0;
    frame.size.width = self.view.bounds.size.width;
    frame.size.height = self.view.frame.size.height - self.tabHeight - kNavigationHeight;
...
- (void)reloadData {
    ...
    if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)]) {
        ...
        _tabWidth = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabWidth withDefault:self.tabWidth];
